...
RECOMMENDED SETUP -- train once, deploy fast
----------------------------------------------
Training on every cold start is what causes free-tier hosts to hang or
crash-loop under memory pressure. Instead:

  1. Run amr_pipeline.py once, somewhere with real compute (e.g. Google
     Colab with a GPU). That produces model_cache/trained_artifacts.pkl,
     bundling the trained classifier AND the parsed CARD data together.
  2. Copy that file into this app's repo, in a folder named `model_cache/`
     next to this script (i.e. `model_cache/trained_artifacts.pkl`).
  3. This app will detect it automatically and load it directly (a few
     seconds), skipping CARD download + parsing + training entirely. It
     still loads the language model itself, for embedding new pasted-in
     sequences -- that part is comparatively light.

If no cached file is found, the app falls back to training from scratch on
first load (like before) -- slow, and easy to exceed a tight free-tier RAM
limit.

HOSTING -- Streamlit Community Cloud vs. Hugging Face Spaces
----------------------------------------------------------------
Streamlit Community Cloud's free tier guarantees only ~1GB RAM (shared,
fractional CPU). Hugging Face Spaces' free "CPU Basic" tier gives 2 vCPUs
and 16GB RAM, natively supports Streamlit apps, and is run by the same
company hosting the Nucleotide Transformer model this app uses. With the
cached artifacts above, either host should work; without them, Spaces has
much more headroom to survive a cold-start training run.

Deploy on Streamlit Community Cloud:
  1. Put this file, requirements.txt, and (ideally) model_cache/trained_artifacts.pkl in your GitHub repo
  2. streamlit.io/cloud -> New app -> point at this file

Deploy on Hugging Face Spaces:
  1. huggingface.co/new-space -> SDK: Streamlit
  2. Upload this file, requirements.txt, and model_cache/trained_artifacts.pkl
     (use git lfs if the pickle file is over 10MB)
"""
...
MAX_TOKENS = 400
BATCH_SIZE = 8
CACHE_PATH = os.path.join("model_cache", "trained_artifacts.pkl")
...
def load_pipeline(model_name=DEFAULT_MODEL_NAME, top_n_classes=TOP_N_CLASSES):
    ...
    # The language model is always needed -- either to build training
    # embeddings (cache miss) or just to embed new pasted-in sequences
    # later (cache hit). Load it once, up front, either way.
    ...
    if os.path.exists(CACHE_PATH):
        # Fast path: a pre-trained bundle exists (produced by running
        # amr_pipeline.py once, elsewhere, and committing the result here).
        # Skips CARD download, parsing, and training entirely.
        with open(CACHE_PATH, "rb") as f:
            cached = pickle.load(f)
        data = cached["data"]
        top_classes = cached["top_classes"]
        label_encoder = cached["label_encoder"]
        llm_clf = cached["clf"]
    else:
        # Slow path: train from scratch. Only recommended on hosts with
        # generous free RAM (e.g. Hugging Face Spaces' 16GB CPU Basic tier) --
        # see the module docstring for how to avoid this entirely.
        download_card()
        data = parse_card_data()
        train_df, _test_df, top_classes, label_encoder = build_train_test(data, top_n_classes=top_n_classes)

        embed_fn_batch = functools.partial(
            embed_sequences, tokenizer=tokenizer, model=model, device=device,
            max_tokens=MAX_TOKENS, batch_size=BATCH_SIZE,
        )
        X_train_emb = embed_fn_batch(train_df["sequence"].tolist())
...
_using_cache = os.path.exists(CACHE_PATH)
_spinner_msg = (
    "Loading pre-trained model (fast path)..."
    if _using_cache else
    "No pre-trained cache found -- downloading CARD and training from scratch "
    "(can take a long time and a lot of memory on free-tier hosting; see the "
    "module docstring for how to avoid this)..."
)
...
if _using_cache:
    st.success(f"Ready (loaded from pre-trained cache). Trained on {len(top_classes)} drug classes: {', '.join(top_classes)}")
else:
    st.warning(
        f"Ready, but trained from scratch this session (no cache found at `{CACHE_PATH}`). "
        f"Trained on {len(top_classes)} drug classes: {', '.join(top_classes)}"
    )
